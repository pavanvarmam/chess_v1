...
$(document).ready(function () {
    ...
    //Click event for closing models
    $(document).on("mousedown", function(event) {
        if(holdPPModel){
            holdPPModel = false;
            return;
        }
        const $promotionIcon = $(event.target);
        if($promotionIcon.hasClass('pp-item')){
            let promotionPiece = $promotionIcon.attr("class").split(" ")[1];
            if(promotionPiece){
                const movement = move(movementData['from'], movementData['to'], promotionPiece[1]);
                postMove({movement:movement, method:movementData['method'], from:movementData['from'], to:movementData['to']});
                movementData = null;
            }
        }
        removePawnPromotion();
    ...
    $('#board').on("mousedown", function (e) {
        ...
        if ($piece) {
            highlightClick();
            if (movePiece({from:highlightedSquare, to:fromSquare, method:null})) {
    ...
    // Helper function to add hints based on valid moves
    function showHints() {
    ...
    function handleMoveAttempt() {
        movePiece({from:highlightedSquare, to:fromSquare, method:null})
        highlightedSquare = null;
        clearHints();
        removeHighlightClick();
    }
    ...
    function handleSameSquareDrop() {
        if (highlightedSquare === toSquare) {
            ...
            if (movePiece({from:highlightedSquare, to:toSquare, method:null})) {
                highlightedSquare = null;
                clearHints();
                removeHighlightClick();
            } else {
    ...
    function handleDragDrop() {
        if (movePiece({from:fromSquare, to:toSquare, method:'drag-drop'})) {
            highlightedSquare = null;
            clearHints();
            removeHighlightClick();
        } else {
    ...
    function movePiece({from, to, method}) {
        ...
        if(movement['promotion']){
            if(!method) holdPPModel = true;
            movementData = {movement,method, from, to}
            game.undo();
            showPawnPromotion(state.turn);
            return true;
        }

        postMove({ movement, method, from, to });
        return true;
    }

    function postMove({movement, method, from, to}){
        update(
            movement,
            state.currentMoveOnBoard?.after || movement.before, // Ensure safe fallback
            movement.after,
            method
        );
    
        if (from && to) {
            highlightMoves(from, to); // Call only if valid values exist
        }
    ...
    function move(from, to, promotion){
        ...
        try{
            return game.move({ from:from, to:to, promotion: promotion || 'q' , verbose:true});
        }catch(e){
            return null;
        }
    }

    function showPawnPromotion(turn){
        removePawnPromotion();
        const $pawnPromotionEle = `
            <div class="pawn-promotion-container">
                <div class="pp-item ${turn}q"></div>
                <div class="pp-item ${turn}b"></div>
                <div class="pp-item ${turn}n"></div>
                <div class="pp-item ${turn}r"></div>
                <section class="pp-item">
                    <span class="pp-item">X</span>
                </section>
            </div>
        `
        $('#board').append($pawnPromotionEle);
    }

    function removePawnPromotion(){
        const $promotionModel = $('.pawn-promotion-container');
        if($promotionModel){
    ...
    function onMoveHistoryClick(nodeIndex){
        ...
        console.log(movement['id'] - state.currentMoveOnBoard['id']);
        

        // if(Math.abs(movement['id'] - state.currentMoveOnBoard['id']) === 1 && !movement['promotion'] && !state.currentMoveOnBoard['promotion'])
        if(Math.abs(movement['id'] - state.currentMoveOnBoard['id']) !== 1){
            shouldAnimate = false;
        }else{
            if(movement['id'] - state.currentMoveOnBoard['id'] === 1 && movement['promotion'])
                shouldAnimate = false;
            if(movement['id'] - state.currentMoveOnBoard['id'] === -1 && state.currentMoveOnBoard['promotion']){
                shouldAnimate = false;
                console.log(movement);
                console.log(state.currentMoveOnBoard);
            }
        }
        ...
        update(movement, state.currentMoveOnBoard['after'], movement['after'], 'update', shouldAnimate);
